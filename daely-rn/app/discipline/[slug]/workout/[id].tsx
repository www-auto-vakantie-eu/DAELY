...
function getAuroraTokens(activeThemeId: string) {
  ...
  return {
    ...
    auroraTitle: isClassic ? '#0F172A' : (currentTheme.colors.auroraTitle || (isZen ? '#FFFFFF' : isRetro ? '#1B2E6B' : isSahara ? '#7A4E24' : isForce ? '#7F1D1D' : isSapphire ? '#EAF2FF' : isRuby ? '#FFE4EC' : isCoral ? '#7A2E2E' : isMarble ? '#1F2937' : isBordeauxVelvet ? '#FFF1F4' : isChampagneRose ? '#2A1F23' : isIvoryGold ? '#2A261C' : isMineralGreen ? '#163227' : isObsidianGold ? '#FFF7D6' : '#1E1B4B')),
    ...
    auroraHighlight: currentTheme.colors.auroraHighlight || (isZen ? '#E5E7EB' : isRetro ? '#E8622A' : isSahara ? '#E7C99B' : isForce ? '#EF4444' : isSapphire ? '#93C5FD' : isRuby ? '#E45A7A' : isCoral ? '#F9736B' : isMarble ? '#D6D3D1' : isBordeauxVelvet ? '#D4AF37' : isChampagneRose ? '#C6A15B' : isIvoryGold ? '#C6A15B' : isMineralGreen ? '#4F7C68' : isObsidianGold ? '#D4AF37' : '#6B5B95'),
    auroraAccent: currentTheme.colors.auroraAccent || (isRetro ? '#C0392B' : isSahara ? '#C89B72' : isForce ? '#DC2626' : isSapphire ? '#3B82F6' : isRuby ? '#B8325A' : isCoral ? '#E85D75' : isMarble ? '#A8A29E' : isBordeauxVelvet ? '#D4AF37' : isChampagneRose ? '#C6A15B' : isIvoryGold ? '#C6A15B' : isMineralGreen ? '#4F7C68' : isObsidianGold ? '#D4AF37' : '#8B7CF6'),
    ribbonTop: isClassic
...
export default function DisciplineWorkoutDetailScreen() {
  ...
  const { auroraGradient, auroraHighlight, auroraAccent, ribbonTop, ribbonMid, ribbonBlue, ribbonRose, ribbonRight, ribbonHighlight, useThinLines, facetLarge, facetMedium, sapphireGlow } = getAuroraTokens(activeThemeId);
