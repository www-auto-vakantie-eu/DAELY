...
function getClassicGlowTokens(activeThemeId: string) {
  ...
  return {
    isClassic,
    gradient: isClassic ? ['#FFFFFF', '#F8FBFF', '#EFF6FF'] : isForce ? (currentTheme.gradients.aurora || ['#FFFFFF', '#FFF1F2', '#FFE4E6']) : isSahara ? (currentTheme.gradients.aurora || ['#FFFFFF', '#FDF8EF', '#F3E4CF']) : currentTheme.gradients.aurora || ['#E8E6FF', '#D0CCFF', '#B8B4FF'],
    titleColor: isClassic ? '#0F172A' : (currentTheme.colors.auroraTitle || (isSahara ? '#7A4E24' : isForce ? '#7F1D1D' : '#1E1B4B')),
    subtitleColor: isClassic ? '#475569' : (currentTheme.colors.auroraSubtitle || (isSahara ? '#9A6B3A' : isForce ? '#B91C1C' : '#4A3A8C')),
    ...
    secondaryButtonColor: isClassic ? '#2563EB' : isForce ? '#DC2626' : isSahara ? '#C89B72' : '#8B5CF6',
  };
}
